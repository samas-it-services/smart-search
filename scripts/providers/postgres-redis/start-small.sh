...
# @samas/smart-search - PostgreSQL + Redis Small Dataset Launcher
# Launch healthcare showcase with small dataset (10K records)
...
main() {
    ...
    case $command in
        start)
            print_header "PostgreSQL + Redis Small Dataset (10K records)"
...
start_services() {
    print_step "Starting PostgreSQL + Redis services (small dataset)..."
    ...
    # Seed data
    print_step "Seeding healthcare data (small dataset - this may take ~2 minutes)..."
    export USE_ALT_PORTS CONTAINER_SUFFIX
    "$PROJECT_ROOT/scripts/seed-data.sh" healthcare small postgres
    
    # Wait for showcase app
    check_http_health "http://localhost:$SHOWCASE_PORT/api/health" "Showcase Application" 60
    
    print_success "PostgreSQL + Redis small dataset showcase started successfully!"
}
...
show_logs() {
    configure_provider_ports "$PROVIDER"
    print_step "Showing PostgreSQL + Redis logs (small dataset)..."
    cd "$DOCKER_DIR"
